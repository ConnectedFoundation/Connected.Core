...
/// <summary>
/// Alters an existing PostgreSQL table to match the desired schema definition.
/// </summary>
/// <remarks>
...
/// apply the changes. The transaction ensures that table structure stays synchronized with entity
/// definitions while preserving existing data where possible.
/// </remarks>
	...
	: TableTransaction
{
	...
	protected override async Task OnExecute()
	{
		...
		{
			var existing = Context.ExistingSchema.Descriptor.Columns.FirstOrDefault(f => string.Equals(f.Name, column.Name, StringComparison.OrdinalIgnoreCase));
		 ...
		 */
		foreach (var existing in Context.ExistingSchema.Descriptor.Columns)
		{
