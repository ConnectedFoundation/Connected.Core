...
/// <summary>
/// Provides base functionality for Oracle database command execution.
/// </summary>
/// <remarks>
...
/// to the storage operation (containing SQL command text and parameters with colon-prefixed bind
/// variables) and the database connection (managing the Oracle.ManagedDataAccess connection).
/// Derived classes implement specific command execution patterns for reading (SELECT) or writing
/// (INSERT/UPDATE/DELETE) operations. The disposal pattern ensures connections are properly
/// released and resources are cleaned up even in error scenarios. Oracle connections support both
/// pooled and non-pooled modes managed by the Oracle.ManagedDataAccess.Core driver.
/// </remarks>
internal abstract class DatabaseCommand(IStorageOperation operation, IStorageConnection connection) : IStorageCommand
{
	...
	/// </summary>
	/// <value>
	/// The <see cref="IStorageOperation"/> with Oracle command text, colon-prefixed bind variable
	...
	/// </summary>
	/// <value>
	/// The <see cref="IStorageConnection"/> managing the Oracle database connection.
	/// </value>
	...
	/// resources that may require async operations during cleanup. Oracle connections and
	/// commands are released back to the connection pool when disposed.
	/// </remarks>
	...
	/// such as closing readers or releasing Oracle-specific resources like LOB descriptors.
	...
	/// resource cleanup when possible, especially for Oracle LOB and REF CURSOR operations.
	/// </remarks>
	...
	/// for derived class cleanup. It follows the standard .NET disposal pattern. Oracle
	/// connections are returned to the pool when disposed.
	/// </remarks>
