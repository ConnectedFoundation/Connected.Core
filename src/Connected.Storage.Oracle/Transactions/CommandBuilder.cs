...
/// <summary>
/// Base class for building Oracle database command text and parameters from entity operations.
/// </summary>
/// <remarks>
/// This abstract class provides the foundation for generating INSERT, UPDATE, and DELETE commands
/// for Oracle databases. It handles property discovery, parameter creation, type mapping, and
/// command text generation using Oracle-specific syntax including double-quoted identifiers and
/// colon-prefixed bind variables. The class uses a caching mechanism to optimize repeated operations
/// on the same entity types. Derived classes implement specific command generation logic for their
/// respective operations while inheriting common functionality for property handling and parameter
/// management. Oracle parameters use colon prefix (:param) instead of at-sign (@param) and support
/// RETURNING INTO clauses for identity value retrieval.
/// </remarks>
internal abstract class CommandBuilder
{
	...
	/// <summary>
	/// Builds an Oracle storage operation for the specified entity.
	/// </summary>
	/// <param name="entity">The entity to build the command for.</param>
	/// <param name="cancel">Cancellation token for the operation.</param>
	/// <returns>An <see cref="OracleStorageOperation"/> containing the command text and parameters, or null if no operation is needed.</returns>
	public async Task<OracleStorageOperation?> Build(IEntity entity, CancellationToken cancel)
	{
		Entity = entity;
		DiscoverProperties();

		if (TryGetExisting(out OracleStorageOperation? existing))
		{
			if (existing is null)
				return null;
			...
			foreach (var parameter in existing.Parameters)
			{
				...
				{
					result.Parameters.Add(new OracleStorageParameter
					{
	...
	/// </summary>
	/// <param name="cancel">Cancellation token for the operation.</param>
	/// <returns>An <see cref="OracleStorageOperation"/> containing the command text and parameters.</returns>
	protected abstract Task<OracleStorageOperation> OnBuild(CancellationToken cancel);
	...
	protected abstract bool TryGetExisting(out OracleStorageOperation? result);
	...
	/// <param name="cancel">Cancellation token for the operation.</param>
	/// <returns>An <see cref="OracleStorageParameter"/> for the property.</returns>
	protected Task<OracleStorageParameter> CreateParameter(PropertyInfo property, CancellationToken cancel)
	{
	...
	/// <returns>An <see cref="OracleStorageParameter"/> for the property.</returns>
	/// <remarks>
	/// Oracle parameters use colon prefix (:param) instead of at-sign prefix (@param).
	/// Parameter names in the command text must match the parameter name without the prefix.
	/// </remarks>
	protected async Task<OracleStorageParameter> CreateParameter(PropertyInfo property, ParameterDirection direction, CancellationToken cancel)
	{
		var columnName = ColumnName(property);
		var parameterName = columnName;

		var parameter = new OracleStorageParameter
		{
			Direction = direction,
			Name = parameterName,
	...
	/// </summary>
	/// <param name="parameterName">The parameter name (without colon prefix).</param>
